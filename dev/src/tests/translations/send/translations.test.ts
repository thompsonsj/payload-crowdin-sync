...
describe('Translations', () => {
  ...
  describe('fn: updateTranslation', () => {
    ...
    it('updates a Payload article with a *blocks* field translation retrieved from Crowdin', async () => {
      ...
      const blockIds = (
        post['layout'] instanceof Array ? post['layout'].map((block) => block.id) : []
      ) as string[]
      const blockTypes =
    ...
    it('updates a Payload article with a *blocks* field translation retrieved from Crowdin and respects non-localized fields', async () => {
      ...
      const responseFr = {
        layout: {
          ...
          [blockIds[2]]: {
            [blockTypes[2]]: {
              textField: "Contenu du champ de texte dans le bloc à l'index de mise en page 2",
              textareaField: "Contenu du champ Textarea dans le bloc à l'index de mise en page 2",
            },
          },
        },
      }
    ...
    it('updates a Payload article with a *blocks* field translation retrieved from Crowdin and detects no change on the next update attempt', async () => {
      ...
      // we need the ids created by Payload to update the blocks
      const blockIds = (
        post['layout'] instanceof Array ? post['layout'].map((block: any) => block.id) : []
      ) as string[]
      const blockTypes =
        post['layout'] instanceof Array ? post['layout'].map((block: any) => block.blockType) : []
      const responseDe = {
        layout: {
          [blockIds[0]]: {
            [blockTypes[0]]: {
    ...
    it('updates a Payload article with *blocks* rich text translations retrieved from Crowdin', async () => {
      ...
      const responseDeOne = '<p>Rich-Text-Inhalt im Blocklayout bei Index 0 oder 1.</p>'
      const responseDeTwo = '<p>Rich-Text-Inhalt im Blocklayout bei Index 0 oder 1.</p>'
      const responseFrOne =
