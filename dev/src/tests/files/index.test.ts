...
describe(`Crowdin file create, update and delete`, () => {
  ...
  describe(`Collection: ${'nested-field-collection'}`, () => {
    ...
    it('creates files containing fieldType content', async () => {
      ...
      await payload
        ...
        .then(async (article) => {
          ...
          expect(crowdinFiles.find((file) => file.name === 'richTextField.html')).toBeDefined()
    ...
    it('creates files containing `array` fieldType content', async () => {
      ...
      const ids =
        (article['arrayField'] instanceof Array && article['arrayField'].map((item) => item.id)) ||
        ([] as string[])
      const crowdinFiles = await getFilesByDocumentID({
        documentId: `${article.id}`,
        payload,
      ...
      expect(
        crowdinFiles.find((file) => file.name === `arrayField.${ids[0]}.richTextField.html`),
      ).toBeDefined()
      expect(
        crowdinFiles.find((file) => file.name === `arrayField.${ids[1]}.richTextField.html`),
      ).toBeDefined()
      expect(crowdinFiles.find((file) => file.name === 'fields.json')).toBeDefined()
    })
    it('creates files containing `blocks` fieldType content', async () => {
      ...
      const blockIds =
        article['layout'] instanceof Array
          ? article['layout'].map((item) => item.id).filter(isDefined)
          : []
      const blockTypes =
        (article['layout'] instanceof Array && article['layout'].map((item) => item.blockType)) ||
        ([] as string[])
      const arrayIds =
        article['layout'] instanceof Array &&
        article['layout'].length > 0 &&
        (article['layout'][1] as any).messages.map((item: any) => item.id)
      const crowdinFiles = await getFilesByDocumentID({
        documentId: `${article.id}`,
        payload,
      })
