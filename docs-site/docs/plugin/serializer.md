...
This can be done through the plugin configuration. The default options are made available for convenience. The default configuration is completely replaced when passing the configuration, so take care to extend the default configuration if you only want to customise some options.
